...
def _v23_words_from_full_json(payload):
    """
    Convert whisper.cpp JSON token timestamps into continuous subtitle words.

    This version deliberately does NOT use DTW token timestamps.  DTW is
    experimental in whisper.cpp and the previous repair could produce large
    gaps when a DTW point became invalid or jumped.  The normal absolute
    token timestamps (timestamps.from/to) are used first.

    Missing token timestamps are repaired from neighbouring normal token
    timestamps or, as a last resort, the containing segment timestamps.
    The resulting spans are clamped to the segment so one bad token cannot
    move the subtitle timeline backwards or terminate it early.
    """
    words = []
    ...
    for seg in payload.get("transcription", []) or []:
        ...
        # Segment timestamps are the final safety net.
        ...
        if not records:
            # Some whisper.cpp outputs can contain segment text without
            # token timing. Preserve it instead of silently dropping it.
        ...
        # Repair missing NORMAL token timestamps using the closest known
        # normal token timestamps. No DTW values are consulted here.
        known = [
            (i, r["start"], r["end"])
        ...
        if not known:
            if seg_start is not None:
                # Preserve all text as one segment span if token timing is
                # completely absent.
        ...
        for i, rec in enumerate(records):
            ...
            for j in range(i - 1, -1, -1):
                if records[j]["start"] is not None:
                    prev = records[j]
                    break
            ...
            if prev is not None and nxt is not None:
                ...
                if b >= a:
                    ...
                    rec["start"] = float(prev["start"])
                    rec["end"] = float(prev["end"])
            elif prev is not None:
                rec["start"] = float(prev["end"])
                rec["end"] = float(prev["end"]) + 0.05
            elif nxt is not None:
                rec["start"] = max(
                    float(seg_start or 0.0),
                    float(nxt["start"]) - 0.05,
                )
        ...
        # Convert repaired token stream into words while preserving the
        # original whisper token spacing.
        current_text = []
        current_start = None
        current_end = None
        ...
        def flush():
            ...
            if text and current_start is not None and current_end is not None:
                end = max(float(current_start) + 0.05,
                          float(current_end))
        ...
        for rec in records:
            ...
            # Clamp repaired values to the containing segment.
            if seg_start is not None:
                start = max(float(seg_start), float(start))
            if seg_end is not None:
                end = min(float(seg_end), float(end))

            if end < start:
                end = start

            clean = rec["text"].replace("\u2581", " ")
            starts_new_word = bool(clean[:1].isspace())
    ...
    # Final guard: subtitle spans must never run backwards.  Keep the text
    # but repair tiny/negative timing anomalies rather than dropping it.
    cleaned = []
    last_start = 0.0
