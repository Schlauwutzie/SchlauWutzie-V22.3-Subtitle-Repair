...
def draw_schlawutzie_head(frame: Image.Image, amplitude: float, t: float) -> Image.Image:
    ...
    crop = src.crop((350, 650, 650, 950))
    mask = Image.new("L", crop.size, 0)
    md = ImageDraw.Draw(mask)
    md.ellipse((24, 10, 276, 292), fill=255)
    mask = mask.filter(ImageFilter.GaussianBlur(6))
    crop.putalpha(mask)
    W, H = frame.size
    activity = float(np.clip(amplitude, 0.0, 1.0))
    base = int(min(W * 0.40, H * 0.22))
    size = max(150, base + int(base * 0.018 * activity))
    crop = crop.resize((size, size), Image.Resampling.LANCZOS)
    bob = int(math.sin(t * 4.0) * 3 + activity * 5)
    x = (W - size) // 2
    y = int(H * 0.30) + bob
    ...
    rr = int(size * 0.49)
    hd.ellipse((cx-rr, cy-rr, cx+rr, cy+rr), fill=(255, 202, 93, 35 + int(35*activity)), outline=(255, 202, 93, 145), width=max(2, W//500))
    halo = halo.filter(ImageFilter.GaussianBlur(max(2, W//250)))
