...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.7 VIDEO + ANIMATION 9:16"
IN_W, IN_H = 720, 1280
...
FIXED_BRAND_REFERENCE = DEFAULT_IMAGE  # Deine Branding.png ist die untere SchlauWutzie-K.I.-Animation.
...
def make_branding_animation_panel(amplitude: float, t: float, size=(1080, 680)):
    """Create the lower SchlauWutzie K.I. animation from the supplied Branding.png.

    The supplied artwork is the animation source itself. No replacement avatar,
    brain graphic or second HUD is generated. The artwork gets a subtle breathing
    zoom plus voice-reactive glow/wave motion in its existing waveform area.
    """
    W, H = size
    if not FIXED_BRAND_REFERENCE or not Path(FIXED_BRAND_REFERENCE).exists():
        return Image.new("RGB", size, (4, 6, 12))

    src = Image.open(FIXED_BRAND_REFERENCE).convert("RGB")
    # Preserve the whole artwork. Branding.png is landscape 16:9-ish.
    scale = min(W / src.width, H / src.height)
    nw, nh = max(1, int(src.width * scale)), max(1, int(src.height * scale))
    base = src.resize((nw, nh), Image.Resampling.LANCZOS)

    # Dark premium panel around the complete artwork.
    panel = Image.new("RGB", size, (3, 6, 12))
    pulse_zoom = 1.0 + 0.018 * math.sin(2 * math.pi * t / 3.8)
    zw, zh = max(1, int(nw * pulse_zoom)), max(1, int(nh * pulse_zoom))
    art = base.resize((zw, zh), Image.Resampling.LANCZOS)
    x = (W - zw) // 2 + int(4 * math.sin(t * 0.9))
    y = (H - zh) // 2 + int(3 * math.sin(t * 1.3))
    panel.paste(art, (x, y))

    out = panel.convert("RGBA")
    activity = float(np.clip(amplitude, 0.0, 1.0))

    # Voice-reactive glow centered on the actual brain/waveform in the supplied image.
    cx, cy = W // 2, int(H * 0.47)
    glow = Image.new("RGBA", size, (0, 0, 0, 0))
    gd = ImageDraw.Draw(glow, "RGBA")
    radius = int(min(W, H) * (0.25 + 0.025 * activity))
    alpha = 24 + int(65 * activity)
    gd.ellipse((cx-radius, cy-radius, cx+radius, cy+radius), fill=(255, 190, 75, alpha))
    glow = glow.filter(ImageFilter.GaussianBlur(max(10, int(W * 0.025))))
    out = Image.alpha_composite(out, glow)

    # Animate ONLY the existing waveform band of the supplied artwork.
    d = ImageDraw.Draw(out, "RGBA")
    base_y = int(H * 0.49)
    left_x0, left_x1 = int(W * 0.12), int(W * 0.40)
    right_x0, right_x1 = int(W * 0.60), int(W * 0.88)
    for x0, x1 in ((left_x0, left_x1), (right_x0, right_x1)):
        bars = 34
        step = (x1 - x0) / bars
        for i in range(bars):
            xx = int(x0 + (i + .5) * step)
            envelope = 0.25 + 0.75 * math.sin(math.pi * (i+.5) / bars) ** 0.7
            motion = 0.5 + 0.5 * math.sin(i * 1.9 + t * 10.0)
            height = 3 + 38 * activity * envelope * (0.25 + 0.75 * motion)
            d.line((xx, int(base_y-height), xx, int(base_y+height*.10)), fill=(255, 203, 105, 165), width=2)
            if i % 4 == 0:
                d.line((xx, int(base_y-height*.75), xx, int(base_y-height*.15)), fill=(96, 214, 255, 145), width=1)

    return out.convert("RGB")


def compose_video_plus_animation(top_frame: Image.Image, amplitude: float, t: float, out_size=(OUT_W, OUT_H)) -> Image.Image:
    """Final TikTok layout: uploaded video on top, user's animation underneath."""
    W, H = out_size
    # 65% video / 35% animation. Both are always present.
    top_h = int(H * 0.65)
    bottom_h = H - top_h

    canvas = Image.new("RGB", (W, H), (3, 5, 10))
    top = fit_cover(top_frame, (W, top_h))
    canvas.paste(top, (0, 0))

    bottom = make_branding_animation_panel(float(amplitude), t, (W, bottom_h))
    canvas.paste(bottom, (0, top_h))

    # Thin separator so the two source layers read as one deliberate layout.
    draw = ImageDraw.Draw(canvas, "RGBA")
    draw.rectangle((0, top_h-2, W, top_h+2), fill=(214, 174, 86, 220))
    return canvas


def render_source_mp4(source_path: str, audio_path: str, output_path: str, progress=None):
    """Render exactly: uploaded video TOP + supplied SchlauWutzie animation BOTTOM.

    The top video remains the user's source. The bottom is always the supplied
    assets/Branding.png animated to the selected audio. Final output is 1080x1920.
    """
    ff = ffmpeg_path()
    ...
    workdir = Path(tempfile.mkdtemp(prefix="schlawutzie_split_"))
    frames_dir = workdir / "frames"
    frames_dir.mkdir(parents=True, exist_ok=True)
    src_pattern = str(frames_dir / "src_%06d.jpg")
    out_pattern = str(frames_dir / "out_%06d.jpg")

    try:
        extract = subprocess.run(
            [ff, "-y", "-i", str(source), "-vf", f"fps={FPS},scale={IN_W}:{IN_H}:force_original_aspect_ratio=increase,crop={IN_W}:{IN_H}", "-q:v", "3", src_pattern],
            stdout=subprocess.PIPE, stderr=subprocess.PIPE,
        )
        if extract.returncode != 0:
            raise RuntimeError("Das MP4 konnte nicht gelesen werden.\n\n" + extract.stderr.decode(errors="ignore")[-3000:])

        source_files = sorted(frames_dir.glob("src_*.jpg"))
        if not source_files:
        ...
        frame_count = max(1, int(math.ceil(audio_duration * FPS)))
        _, amps = amplitude_curve(audio_path, frame_count)

        for i in range(frame_count):
            # Hold the last source frame if the chosen voice is longer than the video.
            src_idx = min(i, len(source_files)-1)
            base = Image.open(source_files[src_idx]).convert("RGB")
            frame = compose_video_plus_animation(base, float(amps[min(i, len(amps)-1)]), i / FPS, (OUT_W, OUT_H))
            frame.save(out_pattern % (i + 1), "JPEG", quality=90, optimize=True)
            if progress and (i % 10 == 0 or i == frame_count-1):
                progress(i+1, frame_count)

        cmd = [ff, "-y", "-framerate", str(FPS), "-i", out_pattern, "-i", str(audio_path),
               "-c:v", "libx264", "-preset", "medium", "-crf", "20", "-pix_fmt", "yuv420p",
               "-c:a", "aac", "-b:a", "192k", "-shortest", "-movflags", "+faststart", str(output_path)]
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if result.returncode != 0:
            raise RuntimeError("SchlauWutzie 9:16-Export fehlgeschlagen.\n\n" + result.stderr.decode(errors="ignore")[-3000:])
    finally:
...
class App(tk.Tk):
    ...
    def _load_default_image(self):
        if DEFAULT_IMAGE.exists():
            ...
            self.image_label.config(
                text="Standardbild: assets/Branding.png • SchlauWutzie-K.I.-Animation unten"
            )
    ...
    def load_source_mp4(self):
        ...
        try:
            ...
            self.set_status("MP4 geladen – VIDEO OBEN + SCHLAUWUTZIE-ANIMATION UNTEN • 9:16 bereit.")
        except Exception as exc:
            self.source_mp4_path = None
    ...
    def show_static_preview(self):
        if self.source_mp4_path:
            # For an MP4, show the requested split layout even before animation starts.
            try:
                ff = ffmpeg_path()
                workdir = Path(tempfile.mkdtemp(prefix="schlawutzie_static_mp4_"))
                pattern = str(workdir / "frame_%05d.jpg")
                result = subprocess.run([ff, "-y", "-ss", "0", "-i", self.source_mp4_path, "-frames:v", "1", "-vf", f"scale={IN_W}:{IN_H}:force_original_aspect_ratio=increase,crop={IN_W}:{IN_H}", "-q:v", "3", pattern], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                files = sorted(workdir.glob("frame_*.jpg"))
                if result.returncode == 0 and files:
                    base = Image.open(files[0]).convert("RGB")
                    frame = compose_video_plus_animation(base, 0.0, 0.0, (OUT_W, OUT_H))
                    frame = frame.resize((360, 640), Image.Resampling.LANCZOS)
        ...
        if not self.background_path:
            return
        # Image-only mode uses the supplied Branding.png as the animation area.
        image = Image.open(self.background_path).convert("RGB")
        frame = fit_contain_blur(image, (OUT_W, OUT_H)).resize((360, 640), Image.Resampling.LANCZOS)
        self.preview_photo = ImageTk.PhotoImage(frame)
        self.preview_canvas.delete("all")
        self.preview_canvas.create_image(180, 320, image=self.preview_photo)
    ...
    def preview(self):
        if not self.background_path and not self.source_mp4_path:
            messagebox.showwarning("Quelle fehlt", "Bitte zuerst ein Bild oder MP4 laden.")
            return
        if not self.audio_path:
            messagebox.showwarning("Audio fehlt", "Bitte StefanM erzeugen oder WAV/MP3 laden.")
            return
        self.set_status("Vorschau: VIDEO OBEN + SCHLAUWUTZIE-ANIMATION UNTEN …")
        threading.Thread(target=self._preview_worker, daemon=True).start()

    def _preview_worker(self):
        try:
            ff = ffmpeg_path()
            workdir = Path(tempfile.mkdtemp(prefix="schlawutzie_preview_"))
            pattern = str(workdir / "frame_%05d.jpg")
            try:
                if self.source_mp4_path:
                    result = subprocess.run([ff, "-y", "-ss", "0", "-i", self.source_mp4_path, "-t", "10", "-vf", f"fps={FPS},scale={IN_W}:{IN_H}:force_original_aspect_ratio=increase,crop={IN_W}:{IN_H}", "-q:v", "4", pattern], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    files = sorted(workdir.glob("frame_*.jpg"))
                    ...
                    # If no video is loaded, use the selected image as the top source.
                ...
                for i in range(count):
                    ...
                    if self.source_mp4_path:
                        composed = compose_video_plus_animation(base, float(amps[min(i, len(amps)-1)]), i / FPS, (OUT_W, OUT_H))
                    else:
                        # Image mode: full Branding image remains the main visual, with the same animation treatment.
                        branding = make_branding_animation_panel(float(amps[min(i, len(amps)-1)]), i / FPS, (OUT_W, OUT_H))
                        composed = branding
                    frames.append(composed.resize((360, 640), Image.Resampling.LANCZOS))
    ...
    def export(self):
        ...
        self.set_status(
            "9:16: VIDEO OBEN + SCHLAUWUTZIE-ANIMATION UNTEN + Untertitel werden gerendert …"
        )
    ...
    def _export_done(self, output):
        ...
        self.set_status(
            "9:16: VIDEO OBEN + SCHLAUWUTZIE-ANIMATION UNTEN + Untertitel fertig."
        )
