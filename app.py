...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.4 FINAL BRANDING"
IN_W, IN_H = 720, 1280
...
FIXED_BRAND_REFERENCE = resource_path("assets/schlawutzie_brand_reference_final.png")
...
def draw_fixed_brand_reference(frame: Image.Image) -> Image.Image:
    """Burn the fixed SchlauWutzie reference design into every rendered frame.

    The uploaded reference artwork is used as a permanent visual signature.
    It is scaled proportionally, kept centered, and blended into the frame
    so it remains visible without destroying the source image/video.
    """
    try:
        ref_path = Path(FIXED_BRAND_REFERENCE)
        if not ref_path.exists():
            return frame
        base = frame.convert("RGBA")
        ref = Image.open(ref_path).convert("RGBA")
        W, H = base.size
        target_w = int(min(W * 0.78, 860))
        ratio = target_w / ref.width
        target_h = max(1, int(ref.height * ratio))
        ref = ref.resize((target_w, target_h), Image.Resampling.LANCZOS)

        # Fixed upper-center placement keeps the subtitle-safe area clear.
        x = (W - target_w) // 2
        y = int(H * 0.045)

        # Slight glass blend: permanently burned in, but still readable over
        # both images and moving video.
        alpha = ref.getchannel("A").point(lambda a: int(a * 0.90))
        ref.putalpha(alpha)
        base.alpha_composite(ref, (x, y))
...
def draw_schlawutzie_branding(frame: Image.Image) -> Image.Image:
    """Apply the fixed SCHLAUWUTZIE K.I. brand mark used by the FINAL template."""
    out = frame.convert("RGBA")
    d = ImageDraw.Draw(out, "RGBA")
    W, H = out.size
    sx, sy = W / 941.0, H / 1672.0

    # Fixed lower-center position matching the reference design.
    cx = W // 2
    cy = int(1370 * sy)
    label_w = int(300 * sx)
    label_h = int(72 * sy)
    ...
    x1 = cx + label_w // 2
    y1 = cy + label_h // 2
    radius = int(18 * min(sx, sy))

    # Soft dark glass plate + gold/cyan edge.
    d.rounded_rectangle(
        (x0, y0, x1, y1),
        radius=radius,
        fill=(8, 14, 24, 205),
        outline=(255, 202, 93, 215),
        width=max(2, int(2 * sx)),
    )
    d.line(
        (x0 + int(28*sx), y1 - int(5*sy), x1 - int(28*sx), y1 - int(5*sy)),
        fill=(83, 210, 255, 190),
        width=max(1, int(2*sy)),
    )

    title_font = font(max(22, int(30 * sy)), bold=True)
    sub_font = font(max(11, int(14 * sy)), bold=False)
    title = "SCHLAUWUTZIE K.I."
    sub = "ANALYSE • DENKEN • VERKNÜPFEN"

    tb = d.textbbox((0,0), title, font=title_font)
    sb = d.textbbox((0,0), sub, font=sub_font)
    d.text(
        (cx - (tb[2]-tb[0]) / 2, y0 + int(8*sy)),
        title,
        font=title_font,
        fill=(255, 224, 150, 255),
        stroke_width=max(1, int(1*sy)),
        stroke_fill=(10, 15, 24, 255),
    )
    d.text(
        (cx - (sb[2]-sb[0]) / 2, y0 + int(43*sy)),
        sub,
        font=sub_font,
        fill=(178, 229, 255, 240),
    )
...
def draw_neural_hud(
    ...
    frame = draw_fixed_brand_reference(frame)
    frame = draw_schlawutzie_branding(frame)
    return frame.convert("RGB")
    ...
    def _load_default_image(self):
        if DEFAULT_IMAGE.exists():
            ...
            self.image_label.config(
                text="Standardbild: assets/schlawutzie.png • Branding: SCHLAUWUTZIE K.I."
            )
