...
def _v23_words_from_full_json(payload):
    """Build reliable word spans from whisper.cpp --output-json-full.

    Important repair for V22.3 subtitle drop-outs:
    - The normal token ``timestamps.from/to`` are the primary source.
      They are absolute segment timestamps and remain available even when
      experimental DTW token timing becomes incomplete.
    - DTW ``t_dtw`` is used only when normal token timing is unavailable.
    - DTW values are accepted only when they are plausible for the current
      segment and monotonic. A bad/reset DTW value therefore cannot pull the
      subtitle timeline backwards or make the remaining text disappear.
    - Segment timestamps are the final fallback, so recognized text is never
      silently discarded just because token timing is missing.
    """
    words = []
    current_text = []
    current_start = None
    ...
    def flush():
        ...
        if text and current_start is not None:
            end = float(current_last_end or current_start)
            words.append((
                text,
                float(current_start),
                max(float(current_start) + 0.05, end),
            ))
        current_text = []
        current_start = None
        current_last_end = None
    ...
    def parse_pair(obj):
        ...
        try:
            a = _v22_parse_ts(ts.get("from"))
            b = _v22_parse_ts(ts.get("to"))
            if b >= a:
    ...
    for seg in payload.get("transcription", []) or []:
        ...
        seg_pair = parse_pair(seg)
        seg_start = seg_pair[0] if seg_pair else None
        seg_end = seg_pair[1] if seg_pair else None
        tokens = seg.get("tokens", []) or []

        # Resolve token timing before assembling words. This lets a token
        # with a point DTW timestamp use the next real token start as its end.
        resolved = []
        last_good = previous_start
        ...
        for tok in tokens:
            ...
            # PRIMARY: normal absolute token timestamps.
            pair = parse_pair(tok)
            start = end = None
            if pair:
                start, end = pair

            # SECONDARY: DTW only when normal token timing is absent.
            if start is None:
                dtw = tok.get("t_dtw", -1)
                if isinstance(dtw, (int, float)) and float(dtw) >= 0:
                    candidate = float(dtw) / 100.0
                    plausible = True
                    if seg_start is not None and candidate < seg_start - 0.50:
                        plausible = False
                    if seg_end is not None and candidate > seg_end + 0.50:
                        plausible = False
                    if last_good is not None and candidate + 0.05 < last_good:
                        plausible = False
                    if plausible:
                        start = candidate
                        end = candidate

            # FINAL: segment timing keeps text present even without token timing.
            if start is None and seg_start is not None:
            ...
            # Prevent an occasional malformed token from moving the timeline
            # backwards. Do not alter normal token timestamps unless they are
            # clearly earlier than the previous resolved token.
            if last_good is not None and start + 0.05 < last_good:
                if seg_start is not None and seg_end is not None:
                    start = max(float(seg_start), min(float(seg_end), last_good))
                    end = max(start, end)
                else:
                    start = last_good
                    end = max(start, end)

            resolved.append([text, float(start), float(end)])
            last_good = float(start)

        # Fill point timestamps from the next token or segment end.
        for i, item in enumerate(resolved):
            text, start, end = item
            if end <= start + 1e-6:
                next_start = None
                for j in range(i + 1, len(resolved)):
                    if resolved[j][1] > start + 1e-6:
                        next_start = resolved[j][1]
                        break
                if next_start is not None:
                    end = next_start
                elif seg_end is not None and seg_end > start:
                    end = float(seg_end)
                else:
                    end = start + 0.05
            resolved[i][2] = max(float(start) + 0.05, float(end))
        ...
        if not resolved:
            ...
            if seg_text and seg_start is not None:
                ...
                words.append((
                    seg_text,
                    float(seg_start),
                    max(float(seg_start) + 0.05, float(seg_end or seg_start + 0.05)),
                ))
            continue

        for text, start, end in resolved:
            clean = text.replace("\u2581", " ")
            ...
            current_last_end = max(float(current_last_end or end), float(end))

        previous_start = resolved[-1][1]
