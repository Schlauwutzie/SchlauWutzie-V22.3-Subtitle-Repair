...
APP_NAME = "SchlauWutzie K.I. – Video Studio V22.12 FINAL VOICE REACTIVE"
IN_W, IN_H = 720, 1280
...
KI_ANIMATION = resource_path("assets/ki_animation_overlay_base.png")
...
def draw_neural_hud(background: Image.Image, amplitude: float, t: float) -> Image.Image:
    """Overlay the original SchlauWutzie K.I. brain/HUD animation on any
    image or video frame. Only the brain, HUD, waveform and branding are used.
    The waveform is driven by the real voice amplitude."""
    frame = background.convert("RGBA")
    W, H = frame.size
    activity = float(np.clip(amplitude, 0.0, 1.0))

    if KI_ANIMATION.exists():
        overlay = Image.open(KI_ANIMATION).convert("RGBA")
        target_w = min(920, W - 100)
        ...
        # Voice-reactive waveform layer, aligned with the reference brain animation.
        d = ImageDraw.Draw(frame, "RGBA")
        baseline = y + int(target_h * 0.47)
        left = x + int(target_w * 0.02)
        right = x + int(target_w * 0.98)
        center = x + target_w // 2
        gap = int(target_w * 0.23)
        bars = 46
        for side in (-1, 1):
            x0 = left if side < 0 else center + gap // 2
            x1 = center - gap // 2 if side < 0 else right
            span = max(1, x1 - x0)
            for i in range(bars):
                u = (i + 0.5) / bars
                env = 0.18 + 0.82 * math.sin(math.pi * u) ** 0.7
                osc = (0.5 + 0.5 * math.sin(i * 1.73 + t * 10.5) * math.sin(i * 0.71 - t * 5.1))
                height = 5 + 58 * activity * env * (0.35 + 0.65 * osc)
                cx = int(x0 + u * span)
                d.line((cx, baseline-height, cx, baseline+4), fill=(255,202,93,210), width=max(1, int(target_w/700)))
                if i % 3 == 0:
                    d.line((cx, baseline-height*0.75, cx, baseline), fill=(92,213,255,180), width=1)
...
def render_video(background_path: str, audio_path: str, output_path: str, progress=None):
    """Render image OR video background + voice-reactive K.I. animation."""
    rate, data = read_audio_pcm(audio_path)
    duration = len(data) / rate if rate else 0.0
    frame_count = max(1, int(math.ceil(duration * FPS)))
    _, amplitudes = amplitude_curve(audio_path, frame_count)
    ff = ffmpeg_path()

    temp_dir = Path(tempfile.mkdtemp(prefix="schlawutzie_v2212_"))
    try:
        overlay_dir = temp_dir / "overlay"
        overlay_dir.mkdir()
        pattern = str(overlay_dir / "ov_%06d.png")

        # Build transparent animation frames only; the real background remains untouched.
        transparent = Image.new("RGB", (OUT_W, OUT_H), (0,0,0))
        for i in range(frame_count):
            # draw on transparent RGBA canvas by using a black background then extracting only the overlay
            base = Image.new("RGBA", (OUT_W, OUT_H), (0,0,0,0))
            if KI_ANIMATION.exists():
                ov = Image.open(KI_ANIMATION).convert("RGBA")
                tw = min(920, OUT_W-100); th = int(ov.height*tw/ov.width)
                ov = ov.resize((tw,th), Image.Resampling.LANCZOS)
                x=(OUT_W-tw)//2; y=OUT_H-th-70
                base.alpha_composite(ov,(x,y))
                d=ImageDraw.Draw(base,'RGBA')
                baseline=y+int(th*0.47); center=OUT_W//2; gap=int(tw*0.23); left=x+int(tw*.02); right=x+int(tw*.98); bars=46
                for side in (-1,1):
                    x0=left if side<0 else center+gap//2; x1=center-gap//2 if side<0 else right; span=max(1,x1-x0)
                    for j in range(bars):
                        u=(j+.5)/bars; env=.18+.82*math.sin(math.pi*u)**.7; osc=.5+.5*math.sin(j*1.73+i/FPS*10.5)*math.sin(j*.71-i/FPS*5.1)
                        h=5+58*float(np.clip(amplitudes[i],0,1))*env*(.35+.65*osc); cx=int(x0+u*span)
                        d.line((cx,baseline-h,cx,baseline+4),fill=(255,202,93,220),width=2)
                        if j%3==0: d.line((cx,baseline-h*.75,cx,baseline),fill=(92,213,255,190),width=1)
            base.save(pattern % (i+1), "PNG")
            if progress and (i%10==0 or i==frame_count-1): progress(i+1,frame_count)

        # Normalize the chosen image/video to the same 9:16 canvas and overlay the transparent animation.
        if Path(background_path).suffix.lower() in {'.mp4','.mov','.mkv','.avi','.webm'}:
            bg_input=["-stream_loop","-1","-i",background_path]
        else:
            bg_input=["-loop","1","-i",background_path]

        command=[ff,"-y",*bg_input,"-framerate",str(FPS),"-i",pattern,"-i",audio_path,
                 "-filter_complex",f"[0:v]scale={OUT_W}:{OUT_H}:force_original_aspect_ratio=increase,crop={OUT_W}:{OUT_H},setsar=1[bg];[1:v]format=rgba[ov];[bg][ov]overlay=0:0:format=auto[v]",
                 "-map","[v]","-map","2:a","-t",f"{duration:.3f}","-c:v","libx264","-preset","medium","-crf","20","-pix_fmt","yuv420p","-c:a","aac","-b:a","192k","-movflags","+faststart",str(output_path)]
        result=subprocess.run(command,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        if result.returncode!=0:
            raise RuntimeError("MP4-Export fehlgeschlagen.\n\n"+result.stderr.decode(errors='ignore')[-3000:])
    finally:
        shutil.rmtree(temp_dir,ignore_errors=True)
...
def _final_transcribe_words(audio_path: str, status_callback=None):
    try:
        ...
        raise RuntimeError(
            "V22.7: Faster-Whisper konnte nicht geladen werden.\n\n"
    ...
    if missing:
        raise RuntimeError(
            "V22.7: KI-Sprachmodell fehlt.\n\n"
            f"Erwarteter Ordner:\n{model_dir}\n\n"
            "Fehlende Dateien:\n" + "\n".join(missing)
        )

    if status_callback:
        status_callback("V22.7: KI-Sprachmodell wird geladen …")

    try:
        # V22.7: CPU-safe loading. Prefer int8_float32 on modern CTranslate2;
        # fall back to int8 for older bundled runtimes.
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "V22.7: KI-Sprachmodell konnte nicht geladen werden.\n\n"
                    f"Pfad:\n{model_dir}\n\n"
                    f"int8_float32: {type(first_exc).__name__}: {first_exc}\n\n"
                    f"int8: {type(second_exc).__name__}: {second_exc}"
                ) from second_exc
    except Exception as exc:
        raise RuntimeError(
    ...
    def run(pass_no: int):
        if status_callback:
            status_callback(
                f"V22.7: Deutsche Sprache wird analysiert (Durchlauf {pass_no}/2) …"
            )
    ...
    if _final_has_repetition_loop(words):
        if status_callback:
            status_callback("V22.7: Wiederholungsfehler erkannt – zweite KI-Analyse …")
        words = run(2)

    if not words:
        raise RuntimeError("V22.7: Es wurden keine deutschen Wörter erkannt.")
    if _final_has_repetition_loop(words):
        raise RuntimeError(
            "V22.7: Die Spracherkennung erzeugt erneut eine Wiederholungsschleife. "
...
def transcribe_german_to_ass(audio_path: str, status_callback=None, source_text: str = "") -> str:
    ...
    if status_callback:
        status_callback(f"V22.7: {len(groups)} synchronisierte Untertitelblöcke erzeugt.")
    return str(ass_path)
...
def _v22_subtitle_helper_entry(audio_path: str, output_ass: str, status_path: str, prompt_path: str):
    try:
        ...
        _status_write(Path(status_path), "V22.7: Deutsche Auto-Untertitel fertig.")
        return 0
    except Exception as exc:
        _status_write(Path(status_path), "V22.7 FEHLER: " + f"{type(exc).__name__}: {exc}")
        return 1
...
class App(tk.Tk):
    ...
    def load_video(self):
        path = filedialog.askopenfilename(
            title="Hintergrundvideo laden",
            filetypes=[("Video", "*.mp4 *.mov *.mkv *.avi"), ("Alle Dateien", "*.*")],
        )
        if not path:
            return
        self.background_path = path
        self.media_type = "video"
        self.image_label.config(text=f"Video: {Path(path).name}")
        self.set_status("Hintergrundvideo geladen.")
        self._show_media_preview()

    def _show_media_preview(self):
        try:
            if getattr(self, "media_type", "image") == "video":
                ff = ffmpeg_path()
                tmp = Path(tempfile.mkdtemp(prefix="schlawutzie_preview_frame_")) / "frame.jpg"
                r = subprocess.run([ff,"-y","-ss","0","-i",self.background_path,"-frames:v","1","-vf","scale=360:640:force_original_aspect_ratio=decrease,pad=360:640:(ow-iw)/2:(oh-ih)/2:black",str(tmp)],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                if r.returncode != 0: return
                image = Image.open(tmp).convert("RGB")
                shutil.rmtree(tmp.parent, ignore_errors=True)
            else:
                image = fit_cover(Image.open(self.background_path),(360,640))
    ...
    def generate_subtitles(self):
        ...
        self.set_status(
            "V22.7: deutsche Auto-Untertitel werden erzeugt …"
        )
    ...
    def _poll_v22_subtitles(
        ...
        if not diagnostic:
            try:
                ...
                diagnostic = (err or out or b"").decode(errors="replace").strip()
            except Exception:
                diagnostic = ""
        self._subtitle_error(
            diagnostic
            or f"V22.7-Untertitelprozess beendet mit Exit-Code {code}."
        )
